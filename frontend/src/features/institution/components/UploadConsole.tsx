...
export const UploadConsole: React.FC<UploadConsoleProps> = ({
    ...
    const content = (
        <div className="fixed inset-0 z-[99999] flex flex-col animate-in fade-in duration-300">
                                    ...
                                    <table className="w-full text-left border-collapse table-fixed min-w-[3200px]">
                                        <thead className="sticky top-0 z-20 bg-[#111111] shadow-md border-b border-white/10">
                                            <tr>
                                                ...
                                                {/* NEW COLUMNS */}
                                                <th className={`w-64 ${thClass}`} onClick={() => requestSort('personal_email')}>
                                                    Personal Email <SortIcon columnKey="personal_email" />
                                                </th>
                                                <th className={`w-36 ${thClass}`} onClick={() => requestSort('phone_number')}>
                                                    Phone <SortIcon columnKey="phone_number" />
                                                </th>
                                                <th className={`w-28 ${thClass}`} onClick={() => requestSort('gender')}>
                                                    Gender <SortIcon columnKey="gender" />
                                                </th>
                                                <th className={`w-36 ${thClass}`} onClick={() => requestSort('date_of_birth')}>
                                                    DOB <SortIcon columnKey="date_of_birth" />
                                                </th>
                                                <th className={`w-48 ${thClass}`} onClick={() => requestSort('father_name')}>
                                                    Father's Name <SortIcon columnKey="father_name" />
                                                </th>
                                                <th className={`w-24 text-center ${thClass}`} onClick={() => requestSort('admission_year')}>
                                                    Adm. Year <SortIcon columnKey="admission_year" />
                                                </th>
                                                <th className={`w-24 text-center ${thClass}`} onClick={() => requestSort('passout_year')}>
                                                    Pass. Year <SortIcon columnKey="passout_year" />
                                                </th>
                                                <th className={`w-24 text-center ${thClass}`} onClick={() => requestSort('10th_percent')}>
                                                    10th% <SortIcon columnKey="10th_percent" />
                                                </th>
                                                <th className={`w-24 text-center ${thClass}`} onClick={() => requestSort('12th_percent')}>
                                                    12th% <SortIcon columnKey="12th_percent" />
                                                </th>
                                                <th className={`w-24 text-center ${thClass}`} onClick={() => requestSort('active_backlogs')}>
                                                    Backlogs <SortIcon columnKey="active_backlogs" />
                                                </th>
                                            ...
                                            {displayData.map((u, i) => {
                                                ...
                                                return (
                                                    <tr key={i} className={`group hover:bg-[#1a1a1a] transition-colors ${isErr ? 'bg-red-500/[0.04]' : 'bg-[#0f0f0f]'}`}>
                                                        ...
                                                        <td className="p-1 px-2 border-r border-white/5">
                                                            {isEditing ? (
                                                                <CellInput value={u.personal_email} onChange={(v: string) => handleUpdateField(originalIndex, 'personal_email', v)} />
                                                            ) : (
                                                                <CellDisplay value={u.personal_email} diffInfo={u._diff?.personal_email} isNew={u._isNew} className="text-[10px] font-mono text-white/40 truncate block lowercase group-hover:text-white/80 transition-colors" />
                                                            )}
                                                        </td>
                                                        <td className="p-1 px-2 border-r border-white/5">
                                                            {isEditing ? (
                                                                <CellInput value={u.phone_number} onChange={(v: string) => handleUpdateField(originalIndex, 'phone_number', v)} />
                                                            ) : (
                                                                <CellDisplay value={u.phone_number} diffInfo={u._diff?.phone_number} isNew={u._isNew} className="text-[10px] font-bold text-white/60 text-center block" />
                                                            )}
                                                        </td>
                                                        <td className="p-1 px-2 border-r border-white/5">
                                                            {isEditing ? (
                                                                <CellInput value={u.gender} onChange={(v: string) => handleUpdateField(originalIndex, 'gender', v)} />
                                                            ) : (
                                                                <CellDisplay value={u.gender} diffInfo={u._diff?.gender} isNew={u._isNew} className="text-[9px] font-black text-white/60 text-center block uppercase" />
                                                            )}
                                                        </td>
                                                        <td className="p-1 px-2 border-r border-white/5">
                                                            {isEditing ? (
                                                                <CellInput value={u.date_of_birth} onChange={(v: string) => handleUpdateField(originalIndex, 'date_of_birth', v)} />
                                                            ) : (
                                                                <CellDisplay value={u.date_of_birth} diffInfo={u._diff?.date_of_birth} isNew={u._isNew} className="text-[10px] font-bold text-white/50 text-center block" />
                                                            )}
                                                        </td>
                                                        <td className="p-1 px-2 border-r border-white/5">
                                                            {isEditing ? (
                                                                <CellInput value={u.father_name} onChange={(v: string) => handleUpdateField(originalIndex, 'father_name', v)} />
                                                            ) : (
                                                                <CellDisplay value={u.father_name} diffInfo={u._diff?.father_name} isNew={u._isNew} className="text-[10px] font-bold text-white/60 block capitalize" />
                                                            )}
                                                        </td>
                                                        <td className="p-1 px-2 border-r border-white/5">
                                                            {isEditing ? (
                                                                <CellInput type="number" value={u.admission_year} onChange={(v: string) => handleUpdateField(originalIndex, 'admission_year', v)} />
                                                            ) : (
                                                                <CellDisplay value={u.admission_year} diffInfo={u._diff?.admission_year} isNew={u._isNew} className="text-[10px] font-bold text-white/50 text-center block" />
                                                            )}
                                                        </td>
                                                        <td className="p-1 px-2 border-r border-white/5">
                                                            {isEditing ? (
                                                                <CellInput type="number" value={u.passout_year} onChange={(v: string) => handleUpdateField(originalIndex, 'passout_year', v)} />
                                                            ) : (
                                                                <CellDisplay value={u.passout_year} diffInfo={u._diff?.passout_year} isNew={u._isNew} className="text-[10px] font-bold text-white/50 text-center block" />
                                                            )}
                                                        </td>
                                                        <td className="p-1 px-2 border-r border-white/5">
                                                            {isEditing ? (
                                                                <CellInput type="number" step="0.01" value={u["10th_percent"]} onChange={(v: string) => handleUpdateField(originalIndex, '10th_percent', v)} />
                                                            ) : (
                                                                <CellDisplay value={u["10th_percent"]} diffInfo={u._diff?.["10th_percent"]} isNew={u._isNew} className="text-[10px] font-bold text-white/50 text-center block" />
                                                            )}
                                                        </td>
                                                        <td className="p-1 px-2 border-r border-white/5">
                                                            {isEditing ? (
                                                                <CellInput type="number" step="0.01" value={u["12th_percent"]} onChange={(v: string) => handleUpdateField(originalIndex, '12th_percent', v)} />
                                                            ) : (
                                                                <CellDisplay value={u["12th_percent"]} diffInfo={u._diff?.["12th_percent"]} isNew={u._isNew} className="text-[10px] font-bold text-white/50 text-center block" />
                                                            )}
                                                        </td>
                                                        <td className="p-1 px-2 border-r border-white/5">
                                                            {isEditing ? (
                                                                <CellInput type="number" value={u.active_backlogs} onChange={(v: string) => handleUpdateField(originalIndex, 'active_backlogs', v)} />
                                                            ) : (
                                                                <CellDisplay value={u.active_backlogs} diffInfo={u._diff?.active_backlogs} isNew={u._isNew} className="text-[10px] font-bold text-red-400/60 text-center block" />
                                                            )}
                                                        </td>
                                                        {isEditing && (
            ...
            {/* Committing Overlay */}
            {isCommitting && (
                <div className="absolute inset-0 bg-[#050505]/60 backdrop-blur-xl z-[100] flex flex-col items-center justify-center p-20 text-center animate-in fade-in duration-300">
