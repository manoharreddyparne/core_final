            ...
            <div className="p-6 bg-black/40 border border-white/5 rounded-3xl space-y-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-6 gap-6">
                    <div className="space-y-1 relative z-10">
                        ...
                        <input type="number" step="0.1" value={formData.min_cgpa} onChange={e => setFormData({ ...formData, min_cgpa: parseFloat(e.target.value) })} className="w-full bg-transparent border-b border-indigo-500/30 text-white px-1 py-2 text-sm focus:border-indigo-500 outline-none transition-all placeholder:text-gray-700" placeholder="0.0" />
                    </div>
                    <div className="space-y-1 relative z-10">
                        <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-1">UG %</label>
                        <input type="number" step="0.1" value={formData.min_ug_percentage} onChange={e => setFormData({ ...formData, min_ug_percentage: parseFloat(e.target.value) })} className="w-full bg-transparent border-b border-indigo-500/30 text-white px-1 py-2 text-sm focus:border-indigo-500 outline-none transition-all placeholder:text-gray-700" placeholder="0.0" />
                    </div>
                    <div className="space-y-1 relative z-10">
                        <label className="text-[9px] font-black text-emerald-400 uppercase tracking-widest ml-1">Conversion Key</label>
                        <input type="number" step="0.1" value={formData.cgpa_to_percentage_multiplier || 9.5} onChange={e => setFormData({ ...formData, cgpa_to_percentage_multiplier: parseFloat(e.target.value) })} className="w-full bg-transparent border-b border-emerald-500/30 text-emerald-400 px-1 py-2 text-sm focus:border-emerald-500 outline-none transition-all" />
                    ...
                    <div className="space-y-1 relative z-10">
                        ...
                        <input type="number" step="0.1" value={formData.min_10th_percent} onChange={e => setFormData({ ...formData, min_10th_percent: parseFloat(e.target.value) })} className="w-full bg-transparent border-b border-indigo-500/30 text-white px-1 py-2 text-sm focus:border-indigo-500 outline-none transition-all placeholder:text-gray-700" placeholder="0.0" />
                    </div>
                    <div className="space-y-1 relative z-10">
                    ...
                    <div className="space-y-1 relative z-10">
                        ...
                        <input type="number" value={formData.allowed_active_backlogs} onChange={e => setFormData({ ...formData, allowed_active_backlogs: parseInt(e.target.value) })} className="w-full bg-transparent border-b border-indigo-500/30 text-white px-1 py-2 text-sm focus:border-indigo-500 outline-none transition-all placeholder:text-gray-700" placeholder="0" />
                    </div>
                </div>
            </div>
